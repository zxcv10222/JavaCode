...
public class BoardController {
	...
	public String list2(@RequestParam(value = "page", defaultValue = "1") int page,
			@RequestParam(value = "searchText", defaultValue = "") String searchText,
			@RequestParam(value = "type", defaultValue = "") String type, Model model) {
		...
		searchText.replace(">", "&gt");
		// 전체 글 개수
		int total = dao.getTotal2(type, searchText);
		// 페이지 계산을 위한 객체 생성
		PageNavigator navi = new PageNavigator(countPerPage, pagePerGroup, page, total);

		// 목록 읽기
		ArrayList<Board2VO> list = dao.list2(navi.getStartRecord(), countPerPage, type, searchText);
	...
	public HashMap<String, Object> autocomplete(HttpServletRequest request, HttpServletResponse response)
			throws Exception {
